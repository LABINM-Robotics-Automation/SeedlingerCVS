...
def add_image_file_names(otd_df):
    td_df = otd_df.copy()
    nuot = get_number_of_trays()
    for t in range(1, nuot+1):
        h_fns = [name for name in os.listdir(f'./images/horizontal/bandeja{t}')]
        for hfn in h_fns:
            q = int(hfn.split('_C')[1].split('.')[0])
            idx = "A" + hfn.split('_A')[1].split('_')[0]
            td_df.loc[
                (td_df['num_bandeja'] == t) & (td_df['idx'] == idx),
                'h_filename'
            ] = hfn
...
def save_td_df(td_df, fn):
    td_df.to_csv("./" + fn)

def run_seedlinger_evaluation():
    ...
    td4_df = add_image_file_names(td3_df)
    print("Prediction evaluation for a two class-based prediction")
    print("3: Good, 2: Not good")
    td5_0df = get_prediction_evaluation(td4_df, 0)
    save_td_df(td5_0df, "two_class_data.csv")
    res = get_stats(td5_0df)

    print("Prediction evaluation for a three class-based prediction")
    print("1: No presence, 3: Good, 2: Not good")
    td5_1df = get_prediction_evaluation(td4_df, 1)
    save_td_df(td5_1df, "three_class_data.csv")
    res = get_stats(td5_1df)

if __name__=="__main__":
    run_seedlinger_evaluation()
